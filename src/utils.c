...
void	who_is_doing(t_philo *philo, t_action code)
{
	...
	{
		if (code == EAT)
			printf("\033[38;5;214m%ld The Philo num %d \
			is eating\033[0m\n", time, philo->id);
		if (code == SLEEP)
			printf("\033[38;5;129m%ld The Philo num %d is \
			sleeping\033[0m\n", time, philo->id);
		if (code == THINK)
			printf("\033[34m%ld The Philo num %d is \
			thinking\033[0m\n", time, philo->id);
		if (code == GRABLEFT)
			printf("\033[38;5;34m%ld The Philo num %d is \
			taking left fork\033[0m\n", time, philo->id);
		if (code == GRABRIGHT)
			printf("\033[38;5;34m%ld The Philo num %d is \
			taking right fork\033[0m\n", time, philo->id);
	}
